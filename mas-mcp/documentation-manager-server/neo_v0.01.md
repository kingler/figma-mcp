# !! STRICTLY FOLLOW THESE INSTRUCTIONS RULES VIA THE COMMANDS !!

- Every time the user chooses to apply a rule or rules via a command ie "/evaluate_code", explicitly state the rules/instructions/command in the output with an explanation of what the command will do. A lot of the commands includes the uses of a Cline tools to run terminal commands or other editor core functions

```yml
SDLC_Orchestration Rules and Instructions:
        ...
        "/init_project":
          ...
          steps:
            ...
            generate_knowledge_graph:
              ...
              command: "/init_knowledge_graph"
              setup_steps:
                - name: "Install Dependencies"
                  command: "npm install"
                  description: "Install required npm packages for visualization"
                
                - name: "Build Visualization"
                  command: "npm run build"
                  description: "Build the React Flow visualization bundle"

              # Depending on project type:
              cli_choices:
                - name: "Python Project"
                  commands:
                    - "pip install networkx matplotlib toml"  # Install Python dependencies
                    - "python scripts/python_dependency_graph.py"
                
                - name: "React Project"
                  commands:
                    - "npm install @babel/parser @babel/traverse glob"  # Install specific React dependencies
                    - "node scripts/react_dependency_graph.js"
                
                - name: "Vue Project"
                  commands:
                    - "npm install @babel/parser @babel/traverse glob @vue/compiler-sfc"  # Install specific Vue dependencies
                    - "node scripts/vue_dependency_graph.js"
              
              input: "Project codebase"
              output_directory: .context/
              output_file: ".context/initial-knowledge-graph.json"
              
              validation:
                - "Check if dependencies are installed correctly"
                - "Verify visualization bundle is built"
                - "Ensure graph generation completes successfully"
                - "Validate output file structure"

              error_handling:
                - condition: "npm install fails"
                  action: "Retry with --force flag"
                
                - condition: "build fails"
                  action: "Clear cache and rebuild"
                
                - condition: "graph generation fails"
                  action: "Check logs and retry with debug mode"
            ...
            setup_context:
              description: "Initialize and setup CCS (Codebase Context Specification)"
              command: "/init_context"  # Changed from /setup_context to use single command
              steps:
                ...
                - name: "Link Knowledge Graph"
                  action: "link_files"
                  links:
                    - source: ".context/initial-knowledge-graph.json"
                      target: ".context/knowledge_graph/current.json"

                - name: "Update Context Index"
                  action: "update_index"
                  file: ".context/documentation_index.json"
                  entries:
                    - "index.md"
                    - "docs.md"
              ...
              validation:
                ...
                - "Check file contents and front matter"
              ...
              error_handling:
                ...
                - condition: "template generation fails"
                  action: "Verify template variables and retry"
                
                - condition: "knowledge graph linking fails"
                  action: "Check file paths and regenerate if needed"
...
# !! STRICTLY FOLLOW THESE RULE INSTRUCTIONS !!

IMPORTANT:

- Every time you choose to apply a rule or rules via a command, explicitly state the rules/instructions/command in the output. You can abbreviate the rule description to a single word or phrase.
