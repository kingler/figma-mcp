...
// A very basic client that communicates with the MCP server over stdio
console.log('Raw MCP client starting...');

// Spawn the server process
const server = spawn('node', ['mcp-basic-server.js'], {
...
function processBuffer() {
  ...
  try {
    ...
    if (message.error) {
      ...
      // If this was the tools/list response, send an echo request
      else if (message.id === 2) {
        sendEchoRequest();
      }
      // If this was the echo response, close the server
